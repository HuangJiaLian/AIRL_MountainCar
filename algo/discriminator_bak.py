...
@LastEditTime: 2019-10-11 19:25:12
'''
    ...
    def __init__(self, env):
        ...
        with tf.variable_scope('discriminator'):
            ...
            with tf.variable_scope('network') as network_scope:
                prob_1 = self.construct_network(input=expert_s_a) # Expert的输出, 永远认为Expert好
                network_scope.reuse_variables()  # share parameter
                prob_2 = self.construct_network(input=agent_s_a) # G的输出, 永远认为Generator差

            with tf.variable_scope('loss'):
                loss_expert = tf.reduce_mean(tf.log(tf.clip_by_value(prob_1, 0.01, 1))) # 想要训练D让prob_1高
                loss_agent = tf.reduce_mean(tf.log(tf.clip_by_value(1 - prob_2, 0.01, 1))) # 想要训练D让prob_2低
